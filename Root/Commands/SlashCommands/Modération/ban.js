...
module.exports = {
	...
	run: async (client, interaction) => {
		if (!interaction.member.permissions.has('BAN_MEMBERS')) return interaction.followUp({ content: "Tu n'as pas les permissions pour utiliser cette commande", ephemeral: true });
		...
		if (!member.bannable || member.user.id === client.user.id) return interaction.followUp('😅 | Je ne peux pas ban ce membre');
		...
		const ban = new MessageEmbed().setDescription(`**${member.user.tag}** a été bannie de se serveur pour \`${reason}\``).setColor('RANDOM').setFooter({ text: 'Ban Member' }).setTimestamp();
