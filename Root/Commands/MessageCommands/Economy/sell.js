...
module.exports = {
	...
	run: async (client, message) => {
		...
		const rows = new MessageActionRow().addComponents(new MessageSelectMenu().setCustomId('sell').setPlaceholder('Choose one Item').addOptions(itemList));
		if (itemList2.length !== 0) {
			const rows2 = new MessageActionRow().addComponents(new MessageSelectMenu().setCustomId('sell2').setPlaceholder('Choose one Item').addOptions(itemList2));
			message.reply({
