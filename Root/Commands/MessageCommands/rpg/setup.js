...
module.exports = {
    ...
    run: async (client, message) => {
        ...
        const races = await raphael.query(`select * from raphael.races`)
            .then((rows, err) => {
                if(err) throw err
                return rows
            })
        ...
        const Race = new MessageActionRow().addComponents(
            new MessageSelectMenu()
                .setCustomId('race')
                .setPlaceholder('Choose Race')
                .setOptions(racesList)
        )
        const classes = await raphael.query(`select * from raphael.classes`)
            .then((rows, err) => {
                if(err) throw err
                return rows
            })
        ...
        const Classe = new MessageActionRow().addComponents(
            new MessageSelectMenu()
                .setCustomId('classe')
                .setPlaceholder('Choose Classe')
                .setOptions(classesList)
        )
        message.reply({
