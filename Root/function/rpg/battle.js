			...
			if (MVita <= 0) {
				...
				if (intelligence > MIntel) {
					...
					return client.channels
						...
						.send(
							`Félicitations, la bataille est terminée après ${i - 1} tours, ${message.author.username}, il te reste ${UVita}HP, Tu gagne ${MPo.toFixed(2)}Gold et ${MExp.toLocaleString({
					...
					return client.channels
						.resolve('778288246806806558')
						.send(
							`Félicitations, la bataille est terminée après ${i} tours, ${message.author.username}, il te reste ${UVita}HP, Tu gagne ${MPo.toFixed(2)}Gold et ${MExp.toLocaleString({
								minimumFractionDigits: 2,
							})}exp !`
						);
				}
