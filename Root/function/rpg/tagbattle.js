...
const tagbattle = async (client, message, player, player2, monster) => {
    ...
    async function tagFight(atkP1, atkP2 , message) {
        ...
        for(let i = 1; MVita > 0; i++) {
            ...
            if(MAgi > P1Agility) {
                if(Player1Atk !== 0) {
                    if(MAgi % Math.floor(Math.random() * (MAgi - (MAgi / 2)) +1) === 0){
                        Player1Atk = 0
                        client.channels.resolve('778288246806806558').send(`tour ${i}: ${monster['MONSTER_NAME']} à esquiver le coup de ${await client.getUserName(client, player['GUILD_ID'], player['USER_ID'])} !`);
                    }}}
            if(MAgi > P2Agility) {
                if(Player2Atk !== 0) {
                    if(MAgi % Math.floor(Math.random() * (MAgi - (MAgi / 2 )) +1) === 0) {
                        Player2Atk = 0
                        client.channels.resolve('778288246806806558').send(`tour ${i}: ${monster['MONSTER_NAME']} à esquiver le coup de ${await client.getUserName(client, player2['GUILD_ID'], player2['USER_ID'])} !`)
                    }}}
            ...
            if(Player2Hp <= 0) {
                await raphael.query(`update raphael.user set VITALITY = 0 where USER_ID = ${player2['USER_ID']}`)
                    .then((rows, err) => {
                        if(err) throw err
                        return rows
                    });
            ...
            if(MVita <= 0) {
                ...
                if(P1Intelligence > MIntel) {
                    ...
                    client.channels.resolve('778288246806806558').send(`Félicitations, la bataille est terminée après ${i - 1} tours, ${await client.getUserName(client, player['GUILd_ID'], player['USER_ID'])}, il te reste ${U1Vita}HP, Tu gagne ${MPo.toLocaleString({ minimumFractionDigits: 2}) / 2}Gold et ${MExp.toLocaleString({ minimumFractionDigits: 2})}exp !`);
                } else {
                    await leveling(client, message, player);
                    client.channels.resolve('778288246806806558').send(`Félicitations, la bataille est terminée après ${i} tours, ${await client.getUserName(client, player['GUILD_ID'], player['USER_ID'])}, il te reste ${U1Vita}HP, Tu gagne ${MPo.toLocaleString({ minimumFractionDigits: 2}) / 2}Gold et ${MExp.toLocaleString({ minimumFractionDigits: 2})}exp !`);
                }
                if(P2Intelligence > MIntel) {
                    await leveling(client, message, player2);
                    return client.channels.resolve('778288246806806558').send(`Félicitations, la bataille est terminée après ${i - 1} tours, ${await client.getUserName(client, player2['GUILd_ID'], player2['USER_ID'])}, il te reste ${U2Vita}HP, Tu gagne ${MPo.toLocaleString({ minimumFractionDigits: 2}) / 2}Gold et ${MExp.toLocaleString({ minimumFractionDigits: 2})}exp !`);
                } else {
                    await leveling(client, message, player2);
                    return client.channels.resolve('778288246806806558').send(`Félicitations, la bataille est terminée après ${i} tours, ${await client.getUserName(client, player2['GUILD_ID'], player2['USER_ID'])}, il te reste ${U2Vita}HP, Tu gagne ${MPo.toLocaleString({ minimumFractionDigits: 2}) / 2}Gold et ${MExp.toLocaleString({ minimumFractionDigits: 2})}exp !`);
                }}}}
